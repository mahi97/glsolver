...
// run() executes with the GIL released; every std::exception raised by the core (invalid input, malformed
// options, an invariant violation reported as std::logic_error) is returned as status "error" with its message
// instead of propagating, so the Python layer can report it and tests can assert on it.
...
SolverOptions parse_weighted_options(const py::dict& options) {
    ...
    auto get_int = [&](const char* key, int def) -> int {
        ...
        try {
            ...
            throw std::invalid_argument(std::string("solve_weighted: option '") + key +
    ...
    auto get_bool = [&](const char* key, bool def) -> bool {
        ...
        try {
            ...
            throw std::invalid_argument(std::string("solve_weighted: option '") + key + "' must be a bool (got " + option_type_name(v) + ")");
        }
    };
...
void bind_solver_weighted(py::module_& m) {
    m.def(
        ...
        "split witnesses are in the trace ('min_cost_split').");
}
