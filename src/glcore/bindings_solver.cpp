...
SolverOptions parse_options(const py::dict& options) {
    ...
    if (options.contains("routing")) {
        ...
        try {
            ...
            throw std::invalid_argument("solve_general: option 'routing' must be a str (got " + option_type_name(v) + ")");
        }
        if (r == "avoid") o.routing_avoid = true;
        else if (r != "bfs")
            throw std::invalid_argument("solve_general: option 'routing' must be 'bfs' or 'avoid' (got '" + r + "')");
    }
    return o;
}
...
void bind_solver(py::module_& m) {
    m.def(
        ...
        "(docs/optimizations.md). options keys: threads, seed, routing ('bfs'|'avoid'), greedy_contraction, lazy_shift, batch_unused_arcs, "
